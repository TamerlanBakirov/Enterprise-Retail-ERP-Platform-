...
public class OrganizationService : IOrganizationService
{
    ...
    public async Task<List<StoreDto>> GetStoresAsync(bool? isActive)
    {
        var q = "organization/stores";
        if (isActive.HasValue) q += $"?isActive={isActive}";
        return await _api.GetAsync<List<StoreDto>>(q) ?? [];
    }

    public async Task<List<WarehouseDto>> GetWarehousesAsync(bool? isActive)
    {
        var q = "organization/warehouses";
        if (isActive.HasValue) q += $"?isActive={isActive}";
        return await _api.GetAsync<List<WarehouseDto>>(q) ?? [];
    }
}
